...
from src.shapes3d.models import ImageEncoder, ImageDecoder, ProjectionHead, LinearProbe
from src.shapes3d.losses import vicreg_loss, deepcca_loss
import numpy as np
import pandas as pd
...
def train(args):
    ...
    # Models — separate encoders for each modality
    ...
    if args.mode == 'supervised':
        classifier = LinearProbe(args.latent_dim, 4).to(device)
    ...
    if args.mode == 'supervised':
        ...
        if decoder_cp:
            params += list(decoder_cp.parameters())
        if proj_x:
            params += list(proj_x.parameters()) + list(proj_y.parameters())
    ...
    for p in enc_x.parameters():
        p.requires_grad = False
    for p in enc_y.parameters():
        p.requires_grad = False
    enc_x.eval()
    enc_y.eval()
    if args.mode in ['ca', 'joint', 'deepcca']:
    ...
    try:
        ...
        plt.legend(handles=scatter.legend_elements()[0],
                   labels=['Cube', 'Cylinder', 'Sphere', 'Capsule'])
    ...
    for p_size in probe_sizes:
        ...
        probe = LinearProbe(args.latent_dim, 4).to(device)
...
if __name__ == "__main__":
    ...
    parser.add_argument('--data_root', type=str, default='./src/shapes3d')
    ...
    parser.add_argument('--rotation_sigma', type=float, default=0.02)
    ...
    parser.add_argument('--cp_target', type=str, default='y', choices=['x', 'y'],
                        help="Modality to predict in CP mode")
    parser.add_argument('--n_samples', type=int, default=10000)
    parser.add_argument('--lambda_val', type=float, default=1.0)
    parser.add_argument('--save_dir', type=str, default='./checkpoints')
    parser.add_argument('--results_csv', type=str, default='checkpoints/shapes3d_results.csv')
